...
| 파일 | 역할 |
| --- | --- |
| `focusable.ts` | 포커스 가능 요소 탐색, 첫 포커스 대상 선정, 최상단 모달 찾기 |
...
- 컨테이너를 넘기지 않으면 `findTopmostModalScope()`로 화면 최상단 모달을 찾는다. 그래서 각 모달이 ref를 배선하지 않아도 동작한다.

> 기준은 **`.ant-modal-wrap`** 하나뿐이다. 내부 컨테이너 클래스는 antd 버전에 따라 바뀌므로(v5 `ant-modal-content` → v6 `ant-modal-container`) 의존하지 않는다. antd를 올린 뒤 자동 포커스가 안 되면 이 셀렉터부터 확인한다.
