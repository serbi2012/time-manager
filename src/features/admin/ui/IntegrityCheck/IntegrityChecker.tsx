  ...
  const runCheck = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (check_result.issues.length === 0) {
        message.success("문제가 발견되지 않았습니다!");
      } else {
        message.warning(
          `${check_result.issues.length}개의 문제가 발견되었습니다`
        );
      }
    }, 500);
  }, [records]);
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (
                  ...
                  // fix_action에서 시간 추출
                  const match = issue.fix_action?.match(/(\d+)분/);
                  if (match) {
                    on_fix_time_mismatch(issue.record_id, parseInt(match[1]));
                  ...
                  on_delete_session(issue.record_id, issue.session_id);
                  message.success("세션이 삭제되었습니다");
                  runCheck();
                } else {
                  message.info("자동 수정 기능이 구현되지 않았습니다");
                }
              }}
            >
              수정
  ...
  return (
    <div>
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              prefix={<BugOutlined />}
              valueStyle={{
                color: result.issues.length > 0 ? "#ff4d4f" : "#52c41a",
              }}
            />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              valueStyle={{
                color: result.error_count > 0 ? "#ff4d4f" : undefined,
              }}
            />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              valueStyle={{
                color: result.warning_count > 0 ? "#faad14" : undefined,
              }}
            />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              valueStyle={{
                color: result.info_count > 0 ? "#1890ff" : undefined,
              }}
            />
          </Card>
        </Col>
      ...
      <Space style={{ marginBottom: 16 }}>
        ...
        <Space.Compact>
          <Button
            type={selected_severity === "all" ? "primary" : "default"}
            onClick={() => setSelectedSeverity("all")}
          >
            전체 ({result.issues.length})
          </Button>
          <Button
            type={selected_severity === "error" ? "primary" : "default"}
          ...
          <Button
            type={selected_severity === "warning" ? "primary" : "default"}
            onClick={() => setSelectedSeverity("warning")}
          >
            경고 ({result.warning_count})
          </Button>
          <Button
            type={selected_severity === "info" ? "primary" : "default"}
      ...
      {filtered_issues.length > 0 ? (
        <Collapse
          defaultActiveKey={Array.from(issues_by_type.keys()).slice(0, 3)}
          items={Array.from(issues_by_type.entries()).map(([type, issues]) => ({
            key: type,
            label: (
              <Space>
                <Badge
                ...
                />
                <Text strong>{ISSUE_TYPE_LABELS[type]}</Text>
              </Space>
            ),
            children: (
              <Table
      ...
      >
        검사 시각: {dayjs(result.checked_at).format("YYYY-MM-DD HH:mm:ss")}
      </Text>
    </div>
  );
}
