...
export function ExportPanel({
  ...
  const preview_stats = useMemo(() => {
    ...
    if (date_range) {
      ...
      filtered = filtered.filter((r) => r.date >= start_str && r.date <= end_str);
    }
  ...
  const handleExport = () => {
    ...
    try {
      ...
      message.success(`${filename} 다운로드 완료`);
    } catch (error) {
        ...
        <Col xs={24} lg={14}>
          <Card title="내보내기 옵션">
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <Paragraph type="secondary" style={{ marginTop: 8 }}>
                {data_type === "records" && "작업 기록 단위로 내보냅니다."}
                {data_type === "sessions" && "개별 세션 단위로 내보냅니다."}
                {data_type === "all" && "레코드와 세션을 모두 포함합니다."}
              </Paragraph>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <Paragraph type="secondary" style={{ marginTop: 8 }}>
                {date_range
                  ? `${date_range[0].format("YYYY-MM-DD")} ~ ${date_range[1].format("YYYY-MM-DD")} 기간의 데이터`
                  : "전체 기간의 데이터를 내보냅니다."}
              </Paragraph>
            </div>
        ...
        <Col xs={24} lg={10}>
          <Card title="내보내기 미리보기">
            <Row gutter={[16, 16]}>
              ...
              <Col span={24}>
                <Statistic
                  title="총 작업 시간"
                  value={formatDuration(preview_stats.total_minutes, time_format)}
                />
              </Col>
            </Row>
            ...
            <Alert
              ...
              description={
                <ul style={{ margin: "8px 0", paddingLeft: 20 }}>
                  <li>형식: {format === "csv" ? "CSV (엑셀 호환)" : "JSON"}</li>
                  <li>
                    데이터:{" "}
                    {data_type === "records"
                      ? "레코드"
                  ...
                  <li>삭제된 데이터: {include_deleted ? "포함" : "제외"}</li>
                </ul>
              }
          ...
          <Card title="도움말" size="small" style={{ marginTop: 16 }}>
            <Paragraph>
              ...
              <br />
              Microsoft Excel, Google Sheets 등에서 바로 열 수 있습니다. 한글이
              깨지는 경우 UTF-8 BOM 인코딩을 지원하는 프로그램을 사용하세요.
            </Paragraph>
            <Paragraph>
              <Text strong>JSON 형식</Text>
              <br />
              전체 데이터를 구조화된 형태로 저장합니다. 데이터 백업이나 다른
              시스템으로 이전할 때 유용합니다.
            </Paragraph>
          </Card>
        </Col>
