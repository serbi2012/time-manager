...
describe('useWorkStore', () => {
    ...
    describe('템플릿 관리', () => {
        ...
        it('reorderTemplates으로 템플릿 순서 변경', () => {
            ...
            const state = useWorkStore.getState()
            expect(state.templates.map((t) => t.id)).toEqual(['t3', 't1', 't2'])
        })
    })
    ...
    describe('자동완성 옵션', () => {
        it('getAutoCompleteOptions는 레코드와 템플릿에서 고유값 추출', () => {
            ...
            const options = useWorkStore.getState().getAutoCompleteOptions(
                'work_name'
            )
        ...
        it('hideAutoCompleteOption으로 옵션 숨김', () => {
            useWorkStore.getState().hideAutoCompleteOption('work_name', '숨길값')
        ...
        it('unhideAutoCompleteOption으로 숨김 해제', () => {
            ...
            useWorkStore.getState().unhideAutoCompleteOption(
                'work_name',
                '숨길값'
            )
    ...
    describe('사용자 정의 옵션', () => {
        ...
        it('중복 업무명 옵션은 추가되지 않음', () => {
            ...
            const state = useWorkStore.getState()
            expect(state.custom_task_options.filter((o) => o === '업무'))
                .toHaveLength(1)
        })
    ...
    describe('템플릿 전환 (switchTemplate)', () => {
        it('진행 중인 작업을 저장하고 새 템플릿으로 전환', () => {
            ...
            expect(useWorkStore.getState().records[0].sessions[0].end_time).toBe('')
    ...
    describe('엣지 케이스', () => {
        ...
        describe('자동완성 엣지 케이스', () => {
            ...
            it('이미 숨김 처리된 옵션을 다시 숨김 처리', () => {
                useWorkStore.getState().hideAutoCompleteOption('work_name', '값')
                useWorkStore.getState().hideAutoCompleteOption('work_name', '값')
