...
| 표시 | 상태 | 설명 |
|------|------|------|
| 🔄 동기화 중... | 진행 중 | 데이터를 서버에 저장하는 중 |
| ☁️ 클라우드 연결됨 | 정상 | 클라우드에 연결됨 |
| ✅ 체크 표시 | 저장 완료 | 변경사항이 저장됨 |
| ⚠️ 동기화 오류 | 오류 | 네트워크 문제 발생 |
| 👤 게스트 모드 | 비로그인 | 로컬 저장 모드 |

> **참고**: 데이터 변경 시 즉시 서버에 저장됩니다. 다른 기기의 변경사항을 확인하려면 "서버에서 새로고침" 기능을 사용하세요.
...
| 위치 | 요소 | 기능 |
|------|------|------|
| 좌측 | 🕐 업무 시간 관리 | 앱 로고 (클릭 시 일간 기록으로 이동) |
| 중앙 | 메뉴 탭 | 페이지 이동 |
...
| 작업 테이블 | 카드 형태 목록 |
| 헤더: 앱 로고만 표시 | 헤더: 현재 페이지명 + 오늘 날짜 표시 |
