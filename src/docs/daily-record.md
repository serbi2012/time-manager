...
| 4    | 거래        | 거래명                            |
| 5    | 카테고리    | `18 환경세팅` 형태의 코드 + 이름  |
| 6    | 카테고리명  | 카테고리 이름만                   |
| 7    | 시간(분)    | 그날 기록한 시간 (숫자만)         |
| 8    | 비고        | 비고                              |
...
> 💡 **코드 입력**: 거래코드는 거래명마다, 카테고리 코드는 카테고리명마다 저장돼요. 한 번 입력해 두면 다음에 같은 이름이 나올 때 자동으로 채워지고, 카테고리는 `18 환경세팅`처럼 코드와 이름을 합쳐 보여줘요. 코드는 이 화면에서만 보여요.
