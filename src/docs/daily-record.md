...
이전에 입력한 값들이 자동완성으로 제안됩니다.

| 기능           | 설명                                                   |
| -------------- | ------------------------------------------------------ |
| **부분 검색**  | 입력한 글자가 포함된 모든 옵션이 표시됨                |
| **하이라이트** | 검색어와 일치하는 부분이 노란색으로 강조됨             |
| **옵션 숨김**  | 불필요한 옵션은 [설정](wiki:settings)에서 숨길 수 있음 |
...
| 요소              | 설명                                     |
| ----------------- | ---------------------------------------- |
| **헤더**          | 제목, ◀ ▶ 날짜 탐색, 총 작업시간 뱃지    |
| **X축**           | 시간 (08:00 ~ 20:00)                     |
| **Y축**           | 작업(거래명) 목록                        |
| **바**            | 각 세션의 시간 범위 (더블클릭으로 수정)  |
| **현재 시간선**   | 빨간 세로선으로 현재 시각 표시           |
| **점심시간 영역** | 점심시간 구간이 반투명 오버레이로 표시됨 |
...
> **팁**: 기존 작업에 추가 모드를 사용하면 같은 작업에 여러 시간대의 세션을 빠르게 추가할 수 있습니다.
...
| 항목               | 설명                                                  |
| ------------------ | ----------------------------------------------------- |
| **세션 시간 수정** | 시작/종료 시간 (HH:mm 형식)                           |
| **작업 정보 수정** | 프로젝트 코드, 작업명, 거래명, 업무명, 카테고리, 비고 |
| **저장 단축키**    | `F8` (설정에서 변경 가능)                             |
| **시간 충돌 검증** | 다른 세션과 시간이 겹치면 저장 전 경고 표시           |
| **진행 중 세션**   | 시작 시간만 수정 가능, 종료 시간은 비활성화           |
